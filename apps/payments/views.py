...
from .serializers import PaymentMethodSerializer, PaystackTransactionSerializer
...
class PaystackInitializeView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        reference = (
            f"APEX-{datetime.now().strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8]}"
        )
        ...
        payload = {
            ...
            "metadata": {
                ...
                "tournament_id": (
                    str(serializer.validated_data.get("tournament", {}).id)
                    if serializer.validated_data.get("tournament")
                    else None
                ),
            },
        }
            ...
            if response.status_code == 200:
                ...
                if data.get("status"):
                    ...
                    transaction_obj = serializer.save(
                        ...
                        gateway_response=payment_data.get(
                            "gateway_response", ""),
                    )

                    self.response_data = {
                        "transaction": PaystackTransactionSerializer(
                            transaction_obj
                        ).data,
    ...
    def get(self, request, reference):
        try:
            transaction_obj = get_object_or_404(
                PaystackTransaction, reference=reference, user=request.user
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("status"):
                    ...
                    transaction_obj.transaction_id = str(
                        payment_data.get("id"))

                    # Handle authorization data safely
                    auth_data = payment_data.get("authorization", {})
                    if isinstance(auth_data, dict):
                        transaction_obj.authorization_code = auth_data.get(
                            "authorization_code", ""
                        )
                    ...
                    if isinstance(auth_data, dict) and auth_data.get(
                        "authorization_code"
                    ):
                        PaymentMethod.objects.get_or_create(
                            user=request.user,
                            authorization_code=auth_data.get(
                                "authorization_code", ""),
                            defaults={
                                "method_type": "CARD",
                    ...
                    transaction_obj.result_description = data.get(
...
class PaystackWebhookView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if event == "charge.success":
            ...
            try:
                transaction_obj = PaystackTransaction.objects.get(
                    reference=reference)
                if transaction_obj.status == "PENDING":
                    ...
                    if (
                        transaction_obj.transaction_type == "PAYMENT" and transaction_obj.tournament
                    ):
...
class PaymentHistoryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return PaystackTransaction.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
...
def withdraw_prize(request):
    ...
    reference = (
        f"PRIZE-{datetime.now().strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8]}"
    )
...
def get_banks(request):
    ...
    try:
        response = requests.get(
            f"{settings.PAYSTACK['BASE_URL']}/bank", headers=headers, timeout=30
        )
