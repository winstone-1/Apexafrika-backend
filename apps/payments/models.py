...
class PaystackTransaction(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.reference} - {self.user.username} - {self.amount} {self.currency}"
        )
...
class PaymentMethod(models.Model):
    ...
    method_type = models.CharField(
        max_length=10, choices=MethodType.choices, default=MethodType.CARD
    )
