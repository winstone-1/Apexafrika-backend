...
from .models import (ContentArticle, ContentArticleComment, ContentArticleView,
                     ContentBookmark, ContentCategory, ContentLearningPath,
                     ContentQuiz, ContentQuizAttempt, ContentReport,
                     ContentSeries, ContentTag, UserLearningProgress)
from .serializers import (ContentArticleCommentSerializer,
                          ContentArticleDetailSerializer,
                          ContentArticleSerializer, ContentBookmarkSerializer,
                          ContentCategorySerializer,
                          ContentLearningPathSerializer,
                          ContentQuizAttemptSerializer, ContentQuizSerializer,
                          ContentReportSerializer, ContentSeriesSerializer,
                          ContentTagSerializer, UserLearningProgressSerializer)
...
class ContentCategoryViewSet(viewsets.ModelViewSet):
    ...
    def articles(self, request, slug=None):
        ...
        serializer = ContentArticleSerializer(
...
class ContentArticleViewSet(viewsets.ModelViewSet):
    ...
    def trending(self, request):
        ...
        articles = ContentArticle.objects.filter(
            status="PUBLISHED", published_at__gte=time_threshold
    ...
    def popular(self, request):
        ...
        articles = ContentArticle.objects.filter(status="PUBLISHED").order_by(
            "-views", "-likes"
        )[:20]
...
class ContentQuizViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def attempt(self, request, pk=None):
        ...
        score = int((correct / len(quiz.questions))
                    * 100) if quiz.questions else 0
...
class ContentLearningPathViewSet(viewsets.ModelViewSet):
    ...
    def enroll(self, request, slug=None):
        ...
        progress, created = UserLearningProgress.objects.get_or_create(
            user=request.user, learning_path=path
        )
        if created:
    ...
    def complete_article(self, request, slug=None):
        ...
        progress = get_object_or_404(
            UserLearningProgress, user=request.user, learning_path=path
        )
