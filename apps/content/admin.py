...
class ContentCategoryAdmin(admin.ModelAdmin):
    list_display = ('name', 'slug', 'type', 'is_active', 'order', 'article_count')
...
class ContentArticleAdmin(admin.ModelAdmin):
    list_display = ('title', 'content_type', 'status', 'author', 'views', 'likes', 'published_at')
    list_filter = ('content_type', 'status', 'is_featured', 'is_educational', 'is_premium', 'language')
    search_fields = ('title', 'summary', 'content')
    prepopulated_fields = {'slug': ('title',)}
    readonly_fields = ('views', 'unique_views', 'likes', 'dislikes', 'shares', 'bookmarks', 'created_at', 'updated_at')
    filter_horizontal = ('categories', 'tags', 'related_articles')
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'slug', 'content', 'summary', 'content_type', 'status')
        }),
        ('Classification', {
            'fields': ('audience', 'language', 'categories', 'tags', 'related_articles')
        }),
        ('Media', {
            'fields': ('featured_image', 'banner_image', 'video_url', 'audio_url')
        }),
        ('Metadata', {
            'fields': ('meta_title', 'meta_description', 'meta_keywords', 'canonical_url', 'schema_markup')
        }),
        ('Feature Flags', {
            'fields': ('is_featured', 'is_educational', 'is_premium', 'is_verified', 'allow_comments', 'allow_sharing')
        }),
        ('Stats', {
            'fields': ('views', 'unique_views', 'likes', 'dislikes', 'shares', 'comments_count', 'bookmarks', 'reading_time')
        }),
        ('Timestamps', {
            'fields': ('published_at', 'reviewed_at', 'reviewed_by', 'created_at', 'updated_at')
        }),
    )

@admin.register(ContentArticleComment)
class ContentArticleCommentAdmin(admin.ModelAdmin):
    list_display = ('article', 'author', 'content_preview', 'is_approved', 'is_flagged', 'created_at')
...
class UserLearningProgressAdmin(admin.ModelAdmin):
    list_display = ('user', 'learning_path', 'status', 'progress', 'last_activity')
    list_filter = ('status',)
...
class ContentReportAdmin(admin.ModelAdmin):
    list_display = ('reporter', 'article', 'report_type', 'status', 'created_at')
