...
class ContentArticleCommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = '__all__'
        read_only_fields = ('created_at', 'updated_at', 'likes', 'dislikes', 'is_approved', 'is_flagged')
    
    def get_replies(self, obj):
        if obj.parent is None:
            return ContentArticleCommentSerializer(obj.replies.filter(is_approved=True), many=True).data
        return []
...
class ContentArticleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = '__all__'
        read_only_fields = (
            'views', 'unique_views', 'likes', 'dislikes', 'shares',
            'comments_count', 'bookmarks', 'created_at', 'updated_at',
    ...
    def get_is_bookmarked(self, obj):
        ...
        if request and request.user.is_authenticated:
            return ContentBookmark.objects.filter(user=request.user, article=obj).exists()
        return False
...
class ContentArticleDetailSerializer(ContentArticleSerializer):
    ...
    class Meta:
        model = ContentArticle
        fields = [
            'id', 'title', 'slug', 'content', 'summary', 'content_type', 
            'status', 'audience', 'language', 'author', 'author_details',
            'categories', 'categories_details', 'tags', 'tags_details',
            'related_articles', 'featured_image', 'banner_image', 
            'video_url', 'audio_url', 'meta_title', 'meta_description',
            'meta_keywords', 'views', 'unique_views', 'likes', 'dislikes',
            'shares', 'comments_count', 'bookmarks', 'reading_time',
            'is_featured', 'is_educational', 'is_premium', 'is_verified',
            'allow_comments', 'allow_sharing', 'canonical_url', 
            'schema_markup', 'published_at', 'reviewed_at', 'reviewed_by',
            'created_at', 'updated_at', 'is_bookmarked', 'series_details'
        ]
...
class ContentBookmarkSerializer(serializers.ModelSerializer):
    ...
    article_details = ContentArticleSerializer(source='article', read_only=True)
...
class UserLearningProgressSerializer(serializers.ModelSerializer):
    ...
    learning_path_details = ContentLearningPathSerializer(source='learning_path', read_only=True)
    user_details = serializers.StringRelatedField(source='user.username')
    completed_articles_details = ContentArticleSerializer(source='completed_articles', many=True, read_only=True)
    
    class Meta:
        model = UserLearningProgress
        fields = '__all__'
        read_only_fields = ('started_at', 'last_activity', 'completed_at', 'certified_at')

class ContentReportSerializer(serializers.ModelSerializer):
    """Serializer for Content Reports"""
    reporter_details = serializers.StringRelatedField(source='reporter.username')
    article_details = ContentArticleSerializer(source='article', read_only=True)
