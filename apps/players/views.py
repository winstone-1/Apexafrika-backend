...
from .serializers import PlayerMatchHistorySerializer, PlayerStatsSerializer
...
class LeaderboardView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if order_by in ["win_rate", "total_wins",
                        "tournaments_won", "total_matches"]:
...
class MatchHistoryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return PlayerMatchHistory.objects.filter(player=self.request.user).order_by(
            "-played_at"
        )
...
def get_player_stats_detail(request, user_id):
    try:
        ...
        return Response(
            {"error": "Player stats not found"}, status=status.HTTP_404_NOT_FOUND
        )
