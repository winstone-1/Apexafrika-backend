...
class Tournament(models.Model):
    ...
    format = models.CharField(
        max_length=20, choices=Format.choices, default=Format.SINGLE_ELIM
    )
    ...
    organizer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="organized_tournaments"
    )
    ...
    banner_image = models.ImageField(
        upload_to="tournament_banners/", blank=True, null=True
    )
    ...
    def is_registration_open(self):
        now = timezone.now()
        return (
            self.status == self.Status.REGISTRATION and self.registration_start <= now <= self.registration_end
        )
